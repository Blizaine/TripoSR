...
import argparse
import aspose.threed as a3d  # Import Aspose.3D for Python via .NET
...
def generate(image, mc_resolution, formats=["obj", "glb", "usdz"]):
    ...
    for format in formats:
        if format.lower() == "usdz":
            temp_glb_path = tempfile.NamedTemporaryFile(suffix=".glb", delete=False).name
            mesh.export(temp_glb_path, file_type='glb')
            scene = a3d.Scene()
            scene.open(temp_glb_path)
            usdz_path = tempfile.NamedTemporaryFile(suffix=".usdz", delete=False).name
            scene.save(usdz_path, a3d.FileFormat.USDZ)
            rv.append(usdz_path)
        else:
            mesh_path = tempfile.NamedTemporaryFile(suffix=f".{format}", delete=False)
            mesh.export(mesh_path.name, file_type=format)
...
def run_example(image_pil):
    ...
    mesh_name_obj, mesh_name_glb, mesh_name_usdz = generate(preprocessed, 256, ["obj", "glb", "usdz"])
    return preprocessed, mesh_name_obj, mesh_name_glb, mesh_name_usdz
...
with gr.Blocks(title="TripoSR") as interface:
    ...
    with gr.Row(variant="panel"):
        ...
        with gr.Column():
            ...
            with gr.Tab("USDZ"):  # Added USDZ tab for download link
                output_model_usdz = gr.File(label="Download USDZ Model", interactive=False, type="filepath")
    with gr.Row(variant="panel"):
        gr.Examples(
            examples=[
    ...
    submit.click(fn=check_input_image, inputs=[input_image]).success(fn=preprocess, inputs=[input_image, do_remove_background, foreground_ratio], outputs=[processed_image]).success(fn=generate, inputs=[processed_image, mc_resolution], outputs=[output_model_obj, output_model_glb, output_model_usdz])  # Updated to include USDZ output
...
if __name__ == '__main__':
    ...
    interface.launch(auth=(args.username, args.password) if (args.username and args.password) else None, share=args.share, server_name="0.0.0.0" if args.listen else None, server_port=args.port)
